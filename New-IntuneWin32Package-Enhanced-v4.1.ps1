...
.NOTES
    Author: Orion
    Version: 4.1 - Fixed $args reserved variable conflict, registry path construction, added transcript logging
...
function Test-SilentInstall {
    ...
    try {
        ...
        $processArgs = if ($fileName -like "*.msi") {
            $Switches + "`"$FilePath`""
        } else {
            $Switches
        }
...
function Find-InstalledAppRegistry {
    ...
    foreach ($Path in $UninstallPaths) {
        ...
        if ($Apps) {
            ...
            $Is32Bit  = $App.PSPath -like "*WOW6432Node*"
            $HivePath = if ($Is32Bit) {
                "HKLM\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall"
            } else {
                "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall"
            }
            $KeyPath = "$HivePath\$KeyName"
...
# Start transcript log alongside CSV report
$transcriptPath = Join-Path $reportsFolder "PackagingLog_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
Start-Transcript -Path $transcriptPath -Force | Out-Null
Write-Log "Transcript logging to: $transcriptPath" -Level Info
